---
title: "Computing the MSE when errors are Laplace Distributed"
...
This code generates Figures 4-6 for the Laplace error case. You need to change the setting to generate all figures as described below.
  ...
  #  print( kappa <-  Var.xi/( Var.xi+sig^2) )
    ...
    for(i in 1:n)  # generate the errors
    {
     ...
     S     <-  matrix( c( Sigma[i,1], Sigma[i,2], Sigma[i,2], Sigma[i,3] ), nrow = 2 , ncol = 2 ) 
      temp  <- rmvlc( samp, c(0,0),U= chol(S) )
